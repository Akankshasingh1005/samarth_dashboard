...
export default function LiveSessionPage() {
  ...
  const {
    ...
    addFeedback, feedbackMessages, resetSession,
  } = useSessionStore();
  ...
  void sensorData; // referenced in PS3 stub section
        ...
        try {
          ...
          if (data.type === 'frame_result') {
            ...
            if (data.rep_count !== undefined && data.rep_count !== repCount) {
              ...
              // Generate mock PS2 per-rep result
  ...
  const handleRepCompleted = (repNumber: number) => {
    // Simulate PS2 per-rep result inline
    ...
    if (errors.length === 0) {
      addFeedback('Good form!');
    } else {
      const msg = `Rep ${repNumber}: ${errors.map(([k]) => ERROR_FLAG_LABELS[k as keyof PS2ErrorFlags].label).join(', ')}`;
      addFeedback(msg);
    }
  };
        ...
        <div className="lg:w-2/5 bg-white border-l border-slate-200 flex flex-col overflow-y-auto">
          ...
          {liveAngles && (
            <div className="p-5 border-b border-slate-100">
              <h3 className="font-display font-bold text-slate-700 mb-4 text-sm uppercase tracking-wide">
                Live Angles
              </h3>
          ...
          {lastRep && (
            <div className="p-5 border-b border-slate-100">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-display font-bold text-slate-700 text-sm uppercase tracking-wide">
                  Last Rep Analysis
                </h3>
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(ERROR_FLAG_LABELS).map(([key, meta]) => {
                  ...
                  return (
                    <div key={key}
                      className={`flex items-center gap-2 p-2 rounded-xl border text-xs transition-all ${flag ? meta.color : 'bg-slate-50 border-slate-200 text-slate-400'}`}>
                      <div>
                        <div className={`font-semibold ${flag ? '' : 'text-slate-400'}`}>{meta.label}</div>
          ...
          <div className="p-5 flex-1">
            <h3 className="font-display font-bold text-slate-700 text-sm uppercase tracking-wide mb-3">
              Feedback Log
            </h3>
            <div className="space-y-2 max-h-48 overflow-y-auto">
              ...
              {feedbackMessages.map((msg, i) => (
                <div key={i} className={`text-xs p-2 rounded-lg ${msg.startsWith('Good') ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700'}`}>
                  {msg}
                </div>
              ))}
            </div>
