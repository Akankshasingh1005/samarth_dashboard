...
def run_pipeline_on_video(video_path, output_subfolder_dir, bg_handler, pose_estimator=None, stride=2, save_annotated_video=False, enhancer=None):
    ...
    # Mock data generation if MediaPipe fails to track
    if landmarks_detected_count == 0:
        num_frames = metadata["frame_count"]
        t = np.linspace(0, num_frames / fps, num_frames)
        squat_wave = np.sin(2 * np.pi * t / 3.5)
        knee_mock = 140.0 + 35.0 * squat_wave
        hip_mock = 150.0 + 25.0 * squat_wave
        ankle_mock = 110.0 + 10.0 * squat_wave
        
        raw_trajectories = {
            "left_knee": list(knee_mock),
            "right_knee": list(knee_mock * 0.98),
            "left_hip": list(hip_mock),
            "right_hip": list(hip_mock * 1.02),
            "left_ankle": list(ankle_mock),
            "right_ankle": list(ankle_mock * 0.99)
        }
